...
import type { ActivityLevel, Client, ClientRecord, DashboardStats, Gender, Goal, Screen, Settings } from "./types";
...
export default function App() {
  ...
  const applySettings = (next: Settings) => {
    ...
    setSettings(merged);
    document.documentElement.style.setProperty("--primary", merged.primary_color || defaultSettings.primary_color);
    document.documentElement.style.setProperty("--app-bg", merged.background_color || defaultSettings.background_color);
  };

  useEffect(() => {
    if (!isDesktopRuntime()) return;
    invoke<Settings>("get_settings").then(applySettings).catch(() => push("تنظیمات خوانده نشد.", "error"));
  }, []);
  ...
  if (!unlocked) return <LoginScreen onLogin={() => setUnlocked(true)} toast={push} toasts={toasts} />;

  return (
    <div className="app-shell min-h-screen bg-[var(--app-bg)] text-charcoal" dir="rtl">
      <div className="flex min-h-screen">
        ...
        <main className="w-full px-5 py-5 md:px-8 lg:px-10">
          ...
          {screen === "dashboard" && <Dashboard version={version} settings={settings} onNew={() => openClientForm()} onCalculator={() => openCalculator()} onEdit={openClientForm} />}
          {screen === "clients" && <Clients version={version} onNew={() => openClientForm()} onEdit={openClientForm} onCalculate={openCalculator} onChanged={() => setVersion((value) => value + 1)} toast={push} />}
          {screen === "client-form" && <ClientForm client={editing} onBack={() => setScreen("clients")} onSaved={(client) => { setEditing(client); setVersion((value) => value + 1); }} toast={push} />}
          {screen === "calculator" && <CalculatorScreen initialClient={calculatorClient} toast={push} />}
          {screen === "settings" && <SettingsScreen settings={settings} setSettings={applySettings} toast={push} />}
        </main>
      </div>
...
function LoginScreen({ onLogin, toast, toasts }: { onLogin: () => void; toast: ToastFn; toasts: Toast[] }) {
  ...
  return (
    <div className="login-shell min-h-screen bg-[var(--app-bg)] text-charcoal" dir="rtl">
...
function Dashboard({ version, settings, onNew, onCalculator, onEdit }: { version: number; settings: Settings; onNew: () => void; onCalculator: () => void; onEdit: (client: Client) => void }) {
  ...
  useEffect(() => {
    if (!isDesktopRuntime()) { setStats({ total_clients: 0, active_clients: 0, recent_clients: [] }); return; }
    setStats(null);
    invoke<DashboardStats>("dashboard_stats").then(setStats).catch(() => setStats({ total_clients: 0, active_clients: 0, recent_clients: [] }));
  }, [version]);
  return (
    <>
      <PageHeader title={settings.dietitian_name ? `سلام، ${settings.dietitian_name}` : "روز آرامی برای مراقبت بهتر"} subtitle={`امروز ${formatPersianDate()} است. پرونده مراجعین و محاسبات تغذیه‌ای شما روی همین دستگاه ذخیره می‌شود.`} action={<PrimaryButton icon={Plus} onClick={onNew}>مراجع جدید</PrimaryButton>} />
      <section className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="card p-6 md:p-8">
          <div className="flex items-start justify-between gap-4">
            <div><p className="text-sm font-semibold text-olive">اقدام سریع</p><h2 className="mt-3 text-2xl font-bold">شروع ویزیت بدون شلوغی</h2><p className="mt-3 max-w-xl text-sm leading-7 text-warm-500">برای ثبت پرونده تازه یا محاسبه سریع نیاز انرژی و ماکروها، همین‌جا شروع کنید.</p></div>
            <Sparkles className="text-sage" size={28} />
          </div>
          <div className="mt-8 grid gap-3 sm:grid-cols-2"><PrimaryButton icon={Plus} onClick={onNew}>ثبت مراجع</PrimaryButton><SecondaryButton icon={Calculator} onClick={onCalculator}>ماشین حساب</SecondaryButton></div>
        </div>
        <div className="grid grid-cols-2 gap-4"><Stat label="همه مراجعین" value={stats?.total_clients} icon={Users} /><Stat label="فعال" value={stats?.active_clients} icon={Leaf} /></div>
      </section>
    ...
    <>
      ...
      <section className="card p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="relative flex-1"><Search className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-warm-500" size={20} /><input className="control w-full pr-12" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="جست‌وجوی نام مراجع" /></div>
          <label className="flex h-12 items-center gap-2 rounded-control border border-warm-100 bg-warm-50 px-4 text-sm text-warm-500"><input type="checkbox" checked={includeArchived} onChange={(event) => setIncludeArchived(event.target.checked)} />نمایش بایگانی</label>
        </div>
        <div className="mt-5 grid gap-3">{!clients ? <SkeletonRows /> : filtered.length === 0 ? <EmptyState icon={Search} title="موردی پیدا نشد" text="نام را تغییر دهید یا مراجع جدید ثبت کنید." /> : filtered.map((client) => <ClientRow key={client.id} client={client} onEdit={() => onEdit(client)} onCalculate={() => onCalculate(client)} onArchive={() => archive(client)} />)}</div>
      </section>
    </>
  );
}
...
function ClientForm({ client, onBack, onSaved, toast }: { client: Client | null; onBack: () => void; onSaved: (client: Client) => void; toast: ToastFn }) {
  ...
  const saveRecord = async () => {
    ...
    try {
      const record = await invoke<ClientRecord>("save_client_record", { record: { client_id: client.id, ...recordForm } });
    ...
    <>
      ...
      <section className="card p-6">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <div className="md:col-span-2"><label className="label">یادداشت پرونده</label><textarea className="control mt-2 min-h-32 w-full py-3" value={form.notes} onChange={(event) => setField("notes", event.target.value)} /></div>
        </div>
        {client?.id && <div className="mt-6 grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-card border border-warm-100 bg-warm-50 p-5">
            <div className="flex items-center gap-2"><ClipboardList size={21} className="text-sage" /><h2 className="text-lg font-bold">رکورد ویزیت امروز</h2></div>
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2"><label className="label">یادداشت ویزیت</label><textarea className="control mt-2 min-h-24 w-full py-3" value={recordForm.notes} onChange={(event) => setRecordForm({ ...recordForm, notes: event.target.value })} /></div>
            </div>
          ...
          <div className="rounded-card border border-warm-100 bg-white p-5">
            <div className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2"><TrendingUp size={21} className="text-sage" /><h2 className="text-lg font-bold">روند وزن</h2></div><span className="text-xs text-warm-500">{formatNumber(records.length)} رکورد</span></div>
...
function CalculatorScreen({ initialClient, toast }: { initialClient: Client | null; toast: ToastFn }) {
  ...
  const choose = (client: Client) => { setSelected(client); setInput(client); setQuery(""); setResults([]); toast("اطلاعات مراجع در ماشین حساب قرار گرفت."); };
  const clear = () => { setSelected(null); setInput(emptyClient); setQuery(""); setOverrides({ calories: "", protein: "", carbs: "", fat: "" }); };
  return (
    <>
      ...
      <div className="grid gap-5 xl:grid-cols-[430px_1fr]">
        <section className="card p-6">
          <div className="mb-5 flex items-center justify-between gap-3"><div><h2 className="text-xl font-bold">ورودی‌ها</h2><p className="helper mt-1">{selected ? `مراجع انتخاب‌شده: ${selected.full_name}` : "حالت ورود دستی فعال است."}</p></div>{selected && <SecondaryButton icon={RotateCcw} onClick={clear}>ورود دستی</SecondaryButton>}</div>
          <div className="relative mb-5"><Search className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-warm-500" size={20} /><input className="control w-full pr-12" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="جست‌وجوی مراجع ذخیره‌شده" />{results.length > 0 && <div className="absolute z-10 mt-2 max-h-72 w-full overflow-auto rounded-card border border-warm-100 bg-paper p-2 shadow-soft">{results.map((client) => <button key={client.id} onClick={() => choose(client)} className="soft-transition flex w-full items-center justify-between rounded-control px-3 py-3 text-right hover:bg-warm-50"><span className="font-semibold">{client.full_name}</span><span className="text-xs text-warm-500">{goalLabels[client.goal]}</span></button>)}</div>}</div>
          <div className="grid gap-4"><SelectField label="جنسیت" value={input.gender} onChange={(value) => setField("gender", value as Gender)} options={genderLabels} /><NumberField label="سن" value={input.age} onChange={(value) => setField("age", value)} suffix="سال" /><NumberField label="قد" value={input.height_cm} onChange={(value) => setField("height_cm", value)} suffix="سانتی‌متر" /><NumberField label="وزن" value={input.weight_kg} onChange={(value) => setField("weight_kg", value)} suffix="کیلوگرم" /><SelectField label="سطح فعالیت" value={input.activity_level} onChange={(value) => setField("activity_level", value as ActivityLevel)} options={activityLabels} /><SelectField label="هدف" value={input.goal} onChange={(value) => setField("goal", value as Goal)} options={goalLabels} /></div>
          <div className="mt-6 rounded-card bg-warm-50 p-4"><p className="text-sm font-bold">اصلاح دستی</p><p className="helper mt-1">کالری یا گرم هر ماکرو را آزادانه تغییر دهید.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><OverrideField label="کالری هدف" value={overrides.calories} onChange={(value) => setOverrides({ ...overrides, calories: value })} /><OverrideField label="پروتئین" value={overrides.protein} onChange={(value) => setOverrides({ ...overrides, protein: value })} /><OverrideField label="کربوهیدرات" value={overrides.carbs} onChange={(value) => setOverrides({ ...overrides, carbs: value })} /><OverrideField label="چربی" value={overrides.fat} onChange={(value) => setOverrides({ ...overrides, fat: value })} /></div></div>
        </section>
        <section className="grid content-start gap-4 sm:grid-cols-2 xl:grid-cols-3"><ResultCard title="BMI" value={formatNumber(calc.bmi, 1)} unit={bmiCategory(calc.bmi)} text="نمای سریع وضعیت وزنی بر اساس قد و وزن." /><ResultCard title="BMR" value={formatNumber(calc.bmr)} unit="کیلوکالری" text="انرژی پایه بدن با فرمول Mifflin-St Jeor." /><ResultCard title="TDEE" value={formatNumber(calc.tdee)} unit="کیلوکالری" text="نیاز روزانه با سطح فعالیت." /><ResultCard title="کالری هدف" value={formatNumber(calories)} unit="کیلوکالری" text="بر اساس هدف وزن و قابل اصلاح دستی." featured /><ResultCard title="پروتئین" value={formatNumber(protein)} unit="گرم" text="پیش‌فرض ۲۵٪ از کالری هدف." /><ResultCard title="کربوهیدرات" value={formatNumber(carbs)} unit="گرم" text="پیش‌فرض ۴۵٪ از کالری هدف." /><ResultCard title="چربی" value={formatNumber(fat)} unit="گرم" text="پیش‌فرض ۳۰٪ از کالری هدف." /></section>
      </div>
    </>
  );
}
...
function SettingsScreen({ settings, setSettings, toast }: { settings: Settings; setSettings: (settings: Settings) => void; toast: ToastFn }) {
  ...
  useEffect(() => { setForm(settings); setCredentials((current) => ({ ...current, username: settings.username || "admin" })); }, [settings]);
  const save = async () => { try { const saved = isDesktopRuntime() ? await invoke<Settings>("save_settings", { settings: form }) : form; setSettings(saved); toast("تنظیمات ذخیره شد."); } catch { toast("ذخیره تنظیمات انجام نشد.", "error"); } };
  ...
  const restoreData = async () => { try { const selected = await open({ multiple: false, filters: [{ name: "Matab Taghzieh backup", extensions: ["json"] }] }); if (!selected || Array.isArray(selected)) return; await invoke("restore_data_backup", { path: selected }); const restored = await invoke<Settings>("get_settings"); setSettings(restored); toast("اطلاعات قبلی بازیابی شد."); } catch { toast("بازیابی انجام نشد.", "error"); } };
  ...
  return (
    <>
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_0.75fr]">
        <div className="card p-6"><div className="grid gap-5 md:grid-cols-2"><TextField label="نام متخصص تغذیه" value={form.dietitian_name} onChange={(value) => setForm({ ...form, dietitian_name: value })} /><TextField label="نام کلینیک" value={form.clinic_name} onChange={(value) => setForm({ ...form, clinic_name: value })} /><ColorField label="رنگ اصلی" value={form.primary_color} onChange={(value) => setForm({ ...form, primary_color: value })} /><ColorField label="رنگ پس‌زمینه" value={form.background_color || defaultSettings.background_color} onChange={(value) => setForm({ ...form, background_color: value })} /></div></div>
        <div className="grid gap-5"><div className="card p-6"><KeyRound className="text-sage" size={28} /><h2 className="mt-5 text-xl font-bold">ورود به برنامه</h2><p className="mt-3 text-sm leading-7 text-warm-500">نام کاربری و رمز اولیه admin است. قبل از تحویل به مشتری تغییر دهید.</p><div className="mt-5 grid gap-4"><TextField label="نام کاربری جدید" value={credentials.username} onChange={(value) => setCredentials({ ...credentials, username: value })} /><PasswordField label="رمز فعلی" value={credentials.current_password} onChange={(value) => setCredentials({ ...credentials, current_password: value })} /><PasswordField label="رمز جدید" value={credentials.password} onChange={(value) => setCredentials({ ...credentials, password: value })} /><PasswordField label="تکرار رمز جدید" value={credentials.repeat} onChange={(value) => setCredentials({ ...credentials, repeat: value })} /></div><div className="mt-6"><SecondaryButton icon={KeyRound} onClick={changeCredentials}>تغییر اطلاعات ورود</SecondaryButton></div></div><div className="card p-6"><Database className="text-sage" size={28} /><h2 className="mt-5 text-xl font-bold">پشتیبان و آپدیت</h2><p className="mt-3 text-sm leading-7 text-warm-500">برای آپدیت فقط فایل JSON سبک را از مشتری بگیرید و بعد از نصب نسخه جدید بازیابی کنید.</p><div className="mt-6 grid gap-3"><SecondaryButton icon={Download} onClick={exportData}>خروجی سبک برای آپدیت</SecondaryButton><SecondaryButton icon={FileUp} onClick={restoreData}>بازیابی اطلاعات قبلی</SecondaryButton><SecondaryButton icon={Database} onClick={exportSqlite}>خروجی کامل SQLite</SecondaryButton></div></div></div>
      </section>
    </>
  );
}

function Brand({ settings }: { settings: Settings }) { return <div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-control bg-[var(--primary)] text-white shadow-lift"><Leaf size={23} /></div><div><p className="text-lg font-bold">{settings.clinic_name || "مطب تغذیه"}</p><p className="mt-1 text-xs text-warm-500">{settings.dietitian_name || "مدیریت حرفه‌ای تغذیه"}</p></div></div>; }
function NavItem({ active, icon: Icon, label, onClick }: { active: boolean; icon: typeof Home; label: string; onClick: () => void }) { return <button onClick={onClick} className={cn("soft-transition flex h-12 items-center gap-3 rounded-control px-4 text-sm font-semibold", active ? "bg-[var(--primary)] text-white shadow-lift" : "text-warm-500 hover:bg-warm-50 hover:text-charcoal")}><Icon size={20} />{label}</button>; }
function MobileNav({ screen, setScreen, openCalculator }: { screen: Screen; setScreen: (screen: Screen) => void; openCalculator: () => void }) { const item = "grid h-11 place-items-center rounded-control border border-warm-100 bg-paper text-warm-500"; return <div className="mb-5 grid grid-cols-4 gap-2 lg:hidden"><button className={cn(item, screen === "dashboard" && "bg-[var(--primary)] text-white")} onClick={() => setScreen("dashboard")} aria-label="داشبورد"><Home size={20} /></button><button className={cn(item, (screen === "clients" || screen === "client-form") && "bg-[var(--primary)] text-white")} onClick={() => setScreen("clients")} aria-label="مراجعین"><Users size={20} /></button><button className={cn(item, screen === "calculator" && "bg-[var(--primary)] text-white")} onClick={openCalculator} aria-label="ماشین حساب"><Calculator size={20} /></button><button className={cn(item, screen === "settings" && "bg-[var(--primary)] text-white")} onClick={() => setScreen("settings")} aria-label="تنظیمات"><SettingsIcon size={20} /></button></div>; }
function PageHeader({ title, subtitle, action }: { title: string; subtitle: string; action?: React.ReactNode }) { return <header className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><h1 className="text-3xl font-bold tracking-normal md:text-4xl">{title}</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-warm-500">{subtitle}</p></div>{action}</header>; }
function PrimaryButton({ children, onClick, icon: Icon, type = "button" }: { children: React.ReactNode; onClick?: () => void; icon?: typeof Plus; type?: "button" | "submit" }) { return <button type={type} onClick={onClick} className="soft-transition inline-flex h-12 items-center justify-center gap-2 rounded-control bg-[var(--primary)] px-5 text-sm font-semibold text-white shadow-lift hover:-translate-y-0.5">{Icon && <Icon size={19} />}{children}</button>; }
function SecondaryButton({ children, onClick, icon: Icon }: { children: React.ReactNode; onClick?: () => void; icon?: typeof Plus }) { return <button onClick={onClick} className="soft-transition inline-flex h-12 items-center justify-center gap-2 rounded-control border border-warm-100 bg-paper px-5 text-sm font-semibold text-charcoal hover:bg-warm-50">{Icon && <Icon size={19} />}{children}</button>; }
function Stat({ label, value, icon: Icon }: { label: string; value?: number; icon: typeof Users }) { return <div className="card p-5"><div className="mb-7 flex items-center justify-between text-warm-500"><span className="text-sm">{label}</span><Icon size={21} /></div>{value === undefined ? <div className="h-10 w-20 animate-pulse rounded-control bg-warm-100" /> : <p className="numbers text-4xl font-bold text-charcoal">{formatNumber(value)}</p>}</div>; }
function ClientRow({ client, onEdit, onCalculate, onArchive }: { client: Client; onEdit: () => void; onCalculate?: () => void; onArchive?: () => void }) { return <div className={cn("rounded-card border border-warm-100 bg-white p-4", client.archived && "opacity-60")}><div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><h3 className="text-lg font-bold">{client.full_name}</h3><p className="mt-2 text-xs text-warm-500">{genderLabels[client.gender]}، {formatNumber(client.age)} سال، {formatNumber(client.height_cm)} سانتی‌متر، {formatNumber(client.weight_kg)} کیلوگرم، {goalLabels[client.goal]}</p></div><div className="flex flex-wrap gap-2">{onCalculate && <SecondaryButton icon={Calculator} onClick={onCalculate}>محاسبه</SecondaryButton>}<SecondaryButton icon={Pencil} onClick={onEdit}>پرونده</SecondaryButton>{onArchive && <SecondaryButton icon={Archive} onClick={onArchive}>{client.archived ? "فعال" : "بایگانی"}</SecondaryButton>}</div></div></div>; }
...
function IconInput({ icon: Icon, label, value, onChange, type = "text", autoComplete }: { icon: typeof UserRound; label: string; value: string; onChange: (value: string) => void; type?: string; autoComplete?: string }) { return <div><label className="label">{label}</label><div className="mt-2 flex h-12 items-center gap-3 rounded-control border border-warm-200 bg-white px-4 focus-within:border-[var(--primary)] focus-within:ring-4 focus-within:ring-emerald/10"><Icon size={19} className="text-warm-500" /><input className="min-w-0 flex-1 border-0 bg-transparent outline-none" type={type} value={value} onChange={(event) => onChange(event.target.value)} autoComplete={autoComplete} /></div></div>; }
function NumberField({ label, value, onChange, suffix }: { label: string; value: number; onChange: (value: number) => void; suffix: string }) { return <div><label className="label">{label}</label><div className="mt-2 flex h-12 items-center rounded-control border border-warm-200 bg-white px-4 focus-within:border-[var(--primary)] focus-within:ring-4 focus-within:ring-emerald/10"><input className="numbers min-w-0 flex-1 border-0 bg-transparent text-left outline-none" type="number" min="0" value={value} onChange={(event) => onChange(Number(event.target.value))} /><span className="text-xs text-warm-500">{suffix}</span></div></div>; }
function DateField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <div><label className="label">{label}</label><input className="control numbers mt-2 w-full text-left" type="date" value={value} onChange={(event) => onChange(event.target.value)} /></div>; }
function ColorField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <div><label className="label">{label}</label><div className="mt-2 flex h-12 items-center gap-3 rounded-control border border-warm-200 bg-white px-3"><Palette size={19} className="text-warm-500" /><input type="color" value={value} onChange={(event) => onChange(event.target.value)} className="h-8 w-12 cursor-pointer border-0 bg-transparent p-0" /><input value={value} onChange={(event) => onChange(event.target.value)} className="numbers min-w-0 flex-1 border-0 bg-transparent text-sm outline-none" /></div></div>; }
function OverrideField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <label className="block"><span className="text-xs font-medium text-warm-500">{label}</span><input className="control numbers mt-1 h-11 w-full text-left" type="number" min="0" value={value} onChange={(event) => onChange(event.target.value)} placeholder="خودکار" /></label>; }
function SelectField({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: Record<string, string> }) { return <div><label className="label">{label}</label><select className="control mt-2 w-full" value={value} onChange={(event) => onChange(event.target.value)}>{Object.entries(options).map(([key, title]) => <option key={key} value={key}>{title}</option>)}</select></div>; }
